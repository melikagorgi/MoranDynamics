# Spatio-temporal Moran dynamics in continuous media

This repository contains simulation and plotting code for the paper:

**Spatio-temporal Moran dynamics in continuous media**

## Contents

- `notebooks/BD.ipynb`: Birth-death update simulations
- `notebooks/DB.ipynb`: Death-birth update simulations
- `notebooks/FK.ipynb`: Fisher-KPP comparison simulations
- `notebooks/plots.ipynb`: Code for generating figures
- `data/`: Saved simulation outputs used for plotting

## Requirements

The code uses Python with NumPy and Matplotlib.

Install requirements with:

```bash
pip install -r requirements.txt
